...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_state:
        ...
        if action == "ADMIN_REJECT_TASK_REASON":
...
async def handle_admin_sub_nav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # INLINE TASK APPROVAL/REJECTION BY ADMIN
        ...
        await query.message.edit_text(f"✅ Task #{t_id} Approved.")
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ticket_step == "ENTERING_TICKET_NET_MESSAGE":
        ...
        with db_transaction() as conn:
            ...
            cursor.execute("INSERT INTO support_tickets (user_id, subject, created_at) VALUES (?, ?, ?)", (user_id, subj, now_str))
            t_id = cursor.lastrowid
            cursor.execute("INSERT INTO support_messages (ticket_id, sender_id, message_text, attachment_file_id, created_at) VALUES (?, ?, ?, ?, ?)", (t_id, user_id, update.message.caption or "Attached File", file_id, now_str))
        safe_clear_state(context)
...
async def handle_regex_routing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # SUB ADMIN: TASK SUBMISSION APPROVALS (Workers Proof)
    ...
    # SUB ADMIN: TASK SUBMISSION REJECTIONS (Workers Proof)
