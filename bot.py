...
def init_db():
    with sqlite3.connect(DATABASE) as conn:
        ...
        CREATE TABLE IF NOT EXISTS submission_reviews (
            submission_id INTEGER PRIMARY KEY, rejected_by INTEGER, rejection_reason TEXT,
            rejection_time TEXT, finalized_by TEXT, finalized_time TEXT, history_logs TEXT
        )""")
        
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS creator_warnings (
            user_id INTEGER PRIMARY KEY, warning_count INTEGER DEFAULT 0, updated_at TEXT
        )""")
...
# =====================================================================
# NAVIGATION & UI (STRICT PRESERVED CHOSEN MENU SECTIONS)
# =====================================================================
async def show_main_menu(update: Update, msg_text: str):
    # Specific ordered grid logic: Job -> Referral -> Profile -> Wallet -> Bonus -> Support
    keyboard = [
        ["📋 Job", "👥 Referral"],
        ["👤 Profile", "💳 Wallet"],
        ["🎁 Bonus", "📞 Support"]
...
async def show_support_menu(update: Update, msg_text: str):
    # Dual-routing sub-branches matching upgraded requirement layout
    keyboard = [
        ["📞 Live Support", "📩 Contact Support"],
        ["🔙 Back"]
    ]
    await update.message.reply_text(msg_text, reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == 'super_admin':
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⚙️ Support Management", callback_data="ad_nav_suppmg")]
        ])
...
async def handle_regex_routing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text.startswith("/dispute_app"):
        if not is_sub_admin(user_id): return
        try: s_id = int(text.replace("/dispute_app", "").strip())
        except: return
        try:
            with db_transaction() as conn:
                review = conn.execute("SELECT finalized_by FROM submission_reviews WHERE submission_id = ?", (s_id,)).fetchone()
                if review and "Admin" in str(review[0]):
                    return await update.message.reply_text("❌ This dispute has already been finalized.")
                
                row = conn.execute("SELECT s.task_id, s.worker_id, t.creator_id, t.reward_amount FROM task_submissions s JOIN tasks t ON s.task_id = t.id WHERE s.id = ?", (s_id,)).fetchone()
                if not row: return await update.message.reply_text("❌ Invalid ID.")
                t_id, w_id, c_id, reward = row
                
                conn.execute("UPDATE task_submissions SET status = 'approved' WHERE id = ?", (s_id,))
                conn.execute("UPDATE users SET earnings_balance = earnings_balance + ? WHERE user_id = ?", (reward, w_id))
                conn.execute("INSERT INTO wallet_transactions (user_id, amount, balance_type, action_type, reference_id, created_at) VALUES (?, ?, 'earnings_balance', 'Admin Dispute Approved', ?, ?)", (w_id, reward, str(s_id), now_str))
                
                conn.execute("INSERT OR IGNORE INTO creator_warnings (user_id, warning_count) VALUES (?, 0)")
                conn.execute("UPDATE creator_warnings SET warning_count = warning_count + 1, updated_at = ? WHERE user_id = ?", (now_str, c_id))
                warn_count = conn.execute("SELECT warning_count FROM creator_warnings WHERE user_id = ?", (c_id,)).fetchone()[0]
                conn.execute("UPDATE submission_reviews SET finalized_by = 'Admin Approved', finalized_time = ?, history_logs = history_logs || ' -> Admin Approved' WHERE submission_id = ?", (now_str, s_id))
                
            await update.message.reply_text(f"✅ Dispute #{s_id} Approved. Task creator warning level is now: {warn_count}")
            try: await context.bot.send_message(chat_id=w_id, text="✅ Admin approved your proof.")
            except: pass
            try: await context.bot.send_message(chat_id=c_id, text="⚠ Warning!\n\nYou rejected a valid proof. Continued offenses lead to suspension.")
            except: pass
        except Exception as e: logger.error(f"Dispute app crash path: {e}")

    elif text.startswith("/dispute_rej"):
        if not is_sub_admin(user_id): return
        try: s_id = int(text.replace("/dispute_rej", "").strip())
        except: return
        try:
            with db_transaction() as conn:
                conn.execute("UPDATE submission_reviews SET finalized_by = 'Admin Rejected', finalized_time = ?, history_logs = history_logs || ' -> Admin Rejected' WHERE submission_id = ?", (now_str, s_id))
                row = conn.execute("SELECT worker_id FROM task_submissions WHERE id = ?", (s_id,)).fetchone()
            await update.message.reply_text(f"❌ Dispute #{s_id} Rejected.")
            try: await context.bot.send_message(chat_id=row[0], text="❌ Admin reviewed your proof.\nFinal Decision: Rejected.")
            except: pass
        except Exception as e: logger.error(f"Dispute rej crash path: {e}")
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ADMIN CONFIGURING DIALOGUE HOOKS ---
    if admin_state:
        action = admin_state.get("action")
        
        if action.startswith("SUPPORT_EDIT_"):
            field_key = action.replace("SUPPORT_EDIT_", "").lower()
            set_setting(f"supp_config_{field_key}", text)
            await update.message.reply_text(f"✅ Support `{field_key}` configuration successfully updated.")
            safe_clear_state(context)
            return

        elif action == "SUPPORT_TICKET_ADMIN_REPLY_INPUT":
            ...
            try:
                ...
                if row:
                    user_push_msg = f"📩 **Support Reply**\n\n🎫 Ticket ID: #{t_id}\n\n💬 {text}"
                    try: await context.bot.send_message(chat_id=target_user, text=user_push_msg)
                    except: pass
                    await update.message.reply_text(f"✅ Reply delivered successfully to User {target_user} for Ticket #{t_id}.")
                safe_clear_state(context)
            except Exception as e: logger.error(f"Admin support reply error: {e}")
            return
            ...
            try:
                with db_transaction() as conn:
                    row = conn.execute("SELECT s.task_id, s.worker_id, s.status, t.creator_id, t.title FROM task_submissions s JOIN tasks t ON s.task_id = t.id WHERE s.id = ?", (s_id,)).fetchone()
                    ...
                    conn.execute("INSERT OR REPLACE INTO submission_reviews (submission_id, rejected_by, rejection_reason, rejection_time, history_logs) VALUES (?, ?, ?, ?, ?)", (s_id, user_id, text, now_str, "Submitted -> Rejected"))
                await update.message.reply_text(f"❌ Worker Submission #{s_id} marked as Rejected.")
                try: await context.bot.send_message(chat_id=row[1], text=f"❌ Your proof for '{row[4]}' was rejected.\n\nReason:\n{text}")
                except: pass
                safe_clear_state(context)
            except Exception as e: logger.error(f"Owner rejection process failure: {e}")
            return
            ...
            except Exception as e: logger.error(f"Task reject error: {e}")
            return
            ...
            try:
                ...
                msg = f"👤 **User [ID: {u[0]}]**\nStatus: {u[5]} | Lvl: {u[6]}\nRef: {u[1]}\nDep: {u[2]} Tk\nEarn: {u[3]} Tk\nPend: {u[4]} Tk"
                kbd = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="ad_nav_users")]])
            ...
            try:
                ...
                await update.message.reply_text(f"✅ {b_field} updated.")
                safe_clear_state(context)
            ...
            await update.message.reply_text(f"✅ Setting `{s_key}` updated.")
            safe_clear_state(context)
            return
            ...
            return await update.message.reply_text(f"✅ Gateway Name registered. Send Account Number:")
            
        elif action == "ADMIN_ADD_METHOD_NUMBER":
            if not is_super_admin(user_id): return
            admin_state["method_number"] = text
            admin_state["action"] = "ADMIN_ADD_METHOD_TYPE"
            return await update.message.reply_text(f"✅ Account Number set. Send Account Type:")
            ...
            try:
                ...
                await update.message.reply_text(f"🎉 Payment Method '{m_name}' successfully added.")
                safe_clear_state(context)
            except Exception: await update.message.reply_text("❌ Error saving gateway.")
            return
    ...
    if withdraw_state == "step_1" and text in ["150 Tk", "300 Tk", "500 Tk", "1000 Tk"]:
        ...
        if user_bal < amount: return await update.message.reply_text(f"❌ Insufficient Earnings!")
        ...
        if len(text) < 11: return await update.message.reply_text("❌ Invalid number!")
        ...
        try:
            ...
            try:
                withdraw_notif_text = f"📤 **New Withdrawal Request**\n\n🆔 ID: #{req_id}\n👤 User: {user_id}\n💰 Amount: {amount} Tk\n🏦 Method: {method}\n📱 No: `{number}`"
                await context.bot.send_message(chat_id=ADMIN_ID, text=withdraw_notif_text)
            except: pass
    ...
    if submission_state == "entering_proof_text_fields":
        if len(text) < 4: return await update.message.reply_text("⚠️ Proof too short:")
        ...
        await update.message.reply_text("📸 Upload Screenshot proof as Image (or type 'Skip'):")
        return
    ...
    if text == "📋 Job":
        safe_clear_state(context)
        await show_tasks_menu(update, "📋 **Job Menu**\n\nনিচের অপশন থেকে আপনার পছন্দের ক্যাটাগরি বেছে নিন:")
        return
    elif text == "📞 Support":
        safe_clear_state(context)
        await show_support_menu(update, "📞 **Support Center**\n\nআপনি কোন উপায়ে আমাদের টিমের সাথে যোগাযোগ করতে চান?")
        return
    elif text == "📞 Live Support":
        safe_clear_state(context)
        context.user_data["ticket_wizard_step"] = "ENTERING_TICKET_SUBJECT"
        await update.message.reply_text("সংক্ষেপে আপনার সমস্যার শিরোনাম (Title) লিখুন:", reply_markup=ReplyKeyboardMarkup([["❌ Cancel"]], resize_keyboard=True))
        return
    elif text == "📩 Contact Support":
        s_text = get_setting("supp_config_text", "সাপোর্ট সেন্টারে স্বাগতম। যেকোনো প্রয়োজনে নিচের বাটনে যোগাযোগ করুন।")
        s_tg = get_setting("supp_config_telegram", "bd94earning")
        s_wp = get_setting("supp_config_whatsapp", "01700000000")
        s_btn = get_setting("supp_config_button", "🌐 Visit Live Channel")
        s_link = get_setting("supp_config_link", "https://t.me/bd94earning")
        
        info_payload = (
            f"📩 **Contact Support Info**\n\n"
            f"{s_text}\n\n"
            f"🔹 Telegram Username: @{s_tg}\n"
            f"🔹 WhatsApp Number: {s_wp}"
        )
        contact_kbd = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"📱 Contact on Telegram", url=f"https://t.me/{s_tg}")],
            [InlineKeyboardButton(f"💬 WhatsApp Support", url=f"https://wa.me/{s_wp}")],
            [InlineKeyboardButton(f"{s_btn}", url=s_link)]
        ])
        await update.message.reply_text(info_payload, reply_markup=contact_kbd, parse_mode="Markdown")
        return
        ...
        ref_reward = int(get_setting("ref_reward", "20"))
        with sqlite3.connect(DATABASE) as conn:
            total_refs = (conn.execute("SELECT referrals FROM users WHERE user_id = ?", (user_id,)).fetchone() or [0])[0]
            successful_refs = (conn.execute("SELECT COUNT(*) FROM users WHERE referrer_id = ? AND referral_paid = 1", (user_id,)).fetchone() or [0])[0]
        pending = max(0, (total_refs - successful_refs) * ref_reward)
        earned = successful_refs * ref_reward
        msg = f"👥 **Referral System Dashboard**\n\n🔗 Your Referral Link:\n`{ref_link}`\n\n📊 Total Referrals: {total_refs}\n✅ Successful Referrals: {successful_refs}\n⏳ Pending Reward: {pending} Tk\n💰 Earned Reward: {earned} Tk"
...
async def handle_admin_nav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); data = query.data.replace("ad_nav_", ""); user_id = query.from_user.id
    if not is_sub_admin(user_id): return
    
    if data == "suppmg":
        kbd = [
            [InlineKeyboardButton("📝 Edit Support Message text", callback_data="ad_sp_edit_text")],
            [InlineKeyboardButton("📱 Edit Telegram User", callback_data="ad_sp_edit_telegram")],
            [InlineKeyboardButton("💬 Edit WhatsApp User", callback_data="ad_sp_edit_whatsapp")],
            [InlineKeyboardButton("🌐 Edit Link Button Name", callback_data="ad_sp_edit_button")],
            [InlineKeyboardButton("🔗 Edit Target URL Link", callback_data="ad_sp_edit_link")],
            [InlineKeyboardButton("🔙 Back to Main Dashboard", callback_data="ad_nav_main")]
        ]
        await query.message.edit_text("⚙️ **Support Configuration Module**\nSelect option:", reply_markup=InlineKeyboardMarkup(kbd))
    elif data == "disputes":
        with sqlite3.connect(DATABASE) as conn:
        ...
        for disp in disputes:
            out += f"🎫 Sub ID: #{disp[0]} | Worker: `{disp[1]}`\n👑 Owner ID: `{disp[3]}`\n💬 Proof Text: {disp[4]}\n❌ Reason: {disp[5]}\n🟢 Approve Worker: `/dispute_app {disp[0]}`\n🔴 Keep Rejected: `/dispute_rej {disp[0]}`\n------------------------\n"
    ...
    elif data == "main": await admin(update, context)

async def handle_admin_sub_nav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; data = query.data; user_id = query.from_user.id; await query.answer()
    
    if data.startswith("ad_sp_edit_"):
        field = data.replace("ad_sp_edit_", "").upper()
        context.user_data["admin_context_state"] = {"action": f"SUPPORT_EDIT_{field}"}
        await query.message.reply_text(f"📥 Send new value for Support **{field}** now:")

    elif data.startswith("ad_task_app_"):
        t_id = int(data.split("_")[-1])
        with db_transaction() as conn:
            conn.execute("UPDATE tasks SET status = 'active' WHERE id = ?", (t_id,))
        await query.message.edit_text(f"✅ Task #{t_id} Approved Successfully.")

    elif data.startswith("ad_task_rej_"):
        t_id = int(data.split("_")[-1])
        context.user_data["admin_context_state"] = {"action": "ADMIN_REJECT_TASK_REASON", "task_id": t_id}
        await query.message.reply_text(f"❌ Rejecting Task #{t_id}. Enter reason:")
        ...
        with db_transaction() as conn:
            ...
            if sub_row and sub_row[2] == 'pending' and sub_row[4] == user_id:
                conn.execute("UPDATE task_submissions SET status = 'approved' WHERE id = ?", (s_id,))
                conn.execute("UPDATE users SET earnings_balance = earnings_balance + ?, total_earned = total_earned + ?, completed_tasks = completed_tasks + 1 WHERE user_id = ?", (sub_row[3], sub_row[3], sub_row[1]))
        ...
        await query.message.reply_text(f"✅ Submission #{s_id} approved.")

    elif data.startswith("own_sub_rej_"):
        s_id = int(data.split("_")[-1])
        context.user_data["admin_context_state"] = {"action": "OWNER_MANDATORY_REJECT_REASON", "sub_id": s_id}
        await query.message.reply_text("Enter rejection reason for this worker proof:")
...
async def admin_fin_chat_sub_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dp_g_mth_"):
        m_id = int(data.replace("dp_g_mth_", ""))
        ...
        await query.message.reply_text(f"📱 Gateway: {gate[0]}\n📞 No: `{gate[1]}`\n\nSend amount deposited:")
        ...
        await query.message.reply_text(f"📝 Reply for support Ticket #{t_id}:")
...
async def handle_browse_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("br_start_task_"):
        ...
        await query.message.delete()
        await context.bot.send_message(chat_id=user_id, text=f"📝 **Task #{t_id} Started!**\n\nPlease submit your required text proofs now:", reply_markup=ReplyKeyboardMarkup([["❌ Cancel"]], resize_keyboard=True))
        return
    ...
    if data.startswith("browse_cat_"):
        ...
        selected_cat = next((k for k in MARKETPLACE_CONFIG.keys() if k.endswith(cat_suffix)), None)
        types = MARKETPLACE_CONFIG.get(selected_cat, [])
        type_counts = {t: 0 for t in types}
        with sqlite3.connect(DATABASE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT task_type, COUNT(*) FROM tasks WHERE category = ? AND status = 'active' AND expires_at > ? AND filled_slots < total_slots AND creator_id != ? AND id NOT IN (SELECT task_id FROM task_submissions WHERE worker_id = ?) GROUP BY task_type", (selected_cat, now_str, user_id, user_id))
            for t_type, cnt in cursor.fetchall():
                if t_type in type_counts: type_counts[t_type] = cnt
        keyboard = [[InlineKeyboardButton(f"{t} ({c})", callback_data=f"br_typ_{selected_cat.split()[-1]}||{t[:15]}")] for t, c in type_counts.items()]
        keyboard.append([InlineKeyboardButton("🔙 Back to Categories", callback_data="browse_back_cats")])
        await query.message.edit_text(f"📁 Category: **{selected_cat}**", reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("br_typ_") or data.startswith("br_pag_"):
        if data.startswith("br_typ_"):
            parts = data.replace("br_typ_", "").split("||")
            cat_p, typ_p, page = parts[0], parts[1], 1
        else:
            parts = data.replace("br_pag_", "").split("||")
            cat_p, typ_p, page = parts[0], parts[1], int(parts[2])
        selected_cat, selected_type = None, None
        for k, v in MARKETPLACE_CONFIG.items():
            if k.endswith(cat_p):
                selected_cat = k
                selected_type = next((t for t in v if t.startswith(typ_p)), None)
                break
        limit = 5
        offset = (page - 1) * limit
        with sqlite3.connect(DATABASE) as conn:
            total_tasks = conn.execute("SELECT COUNT(*) FROM tasks WHERE category = ? AND task_type = ? AND status = 'active' AND expires_at > ? AND filled_slots < total_slots AND creator_id != ? AND id NOT IN (SELECT task_id FROM task_submissions WHERE worker_id = ?)", (selected_cat, selected_type, now_str, user_id, user_id)).fetchone()[0]
            tasks_list = conn.execute("SELECT id, title, reward_amount, total_slots, filled_slots, description FROM tasks WHERE category = ? AND task_type = ? AND status = 'active' AND expires_at > ? AND filled_slots < total_slots AND creator_id != ? AND id NOT IN (SELECT task_id FROM task_submissions WHERE worker_id = ?) ORDER BY id DESC LIMIT ? OFFSET ?", (selected_cat, selected_type, now_str, user_id, user_id, limit, offset)).fetchall()
        
        text_out = f"🔍 **Find Job ({selected_type})**\n\n"
        for tk in tasks_list:
            text_out += f"🆔 Open Task: /viewtask{tk[0]}\n💎 Title: **{tk[1]}**\n💰 Reward: {tk[2]} Tk | Slots: {tk[4]}/{tk[3]}\n------------------------\n"
        nav_buttons = []
        if page > 1: nav_buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"br_pag_{selected_cat.split()[-1]}||{selected_type[:15]}||{page-1}"))
        if offset + limit < total_tasks: nav_buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"br_pag_{selected_cat.split()[-1]}||{selected_type[:15]}||{page+1}"))
        kbd = [nav_buttons] if nav_buttons else []
        kbd.append([InlineKeyboardButton("🔙 Back", callback_data=f"browse_cat_{selected_cat.split()[-1]}")])
        await query.message.edit_text(text_out, reply_markup=InlineKeyboardMarkup(kbd), parse_mode="Markdown")
...
async def handle_wizard_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wz_cat_"):
        ...
        await query.message.edit_text(f"📁 Category: **{cat}**", reply_markup=InlineKeyboardMarkup(kbd))
    elif data.startswith("wz_typ_"):
        t_typ = next((t for t in MARKETPLACE_CONFIG.get(wizard["category"], []) if t.startswith(data.split("wz_typ_")[-1])), None)
        ...
        wizard["proofs_config"].append({"name": wizard["current_proof_label_name"], "type": data.split("wz_p_t_")[-1]})
        kbd = [[InlineKeyboardButton("➡️ Next", callback_data="wz_act_goto_tutorial_image_step")]]
        await query.message.edit_text(f"✅ Proof saved. Proceed:", reply_markup=InlineKeyboardMarkup(kbd))
    elif data == "wz_act_goto_tutorial_image_step":
        wizard["step"] = "TUTORIAL_IMAGE_UPLOAD"
        await query.message.delete()
        await context.bot.send_message(user_id, "📷 Step 6: Upload Tutorial Image:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Skip", callback_data="wz_skp_tutorial_img_upload")]]))
        ...
        wizard["total_budget"] = int(subtotal * 1.1)
        msg = f"📊 **Invoice Preview**\n\nTitle: {wizard['title']}\nWorkers: {wizard['workers']}\nReward: {wizard['reward_amount']} Tk\n💳 Total Budget: **{wizard['total_budget']} Tk**"
        await query.message.delete()
        await context.bot.send_message(user_id, msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ All Done", callback_data="wz_act_publish")], [InlineKeyboardButton("❌ Cancel", callback_data="wz_act_cancel_wizard_pipeline")]]))
    elif data == "wz_act_publish":
        budget = wizard["total_budget"]
        try:
            with db_transaction() as conn:
                cursor = conn.cursor()
                dep_bal = (cursor.execute("SELECT deposit_balance FROM users WHERE user_id = ?", (user_id,)).fetchone() or [0])[0]
                if dep_bal < budget:
                    safe_clear_state(context)
                    return await context.bot.send_message(user_id, "❌ Insufficient Balance.")
                cursor.execute("UPDATE users SET deposit_balance = deposit_balance - ? WHERE user_id = ?", (budget, user_id))
                exp = datetime.datetime.now() + datetime.timedelta(days=int(wizard["time_limit"].split()[0]))
                cursor.execute("INSERT INTO tasks (creator_id, title, description, link, proof_requirements, reward_amount, total_slots, total_budget, status, created_at, category, task_type, tutorial_image, time_limit, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending_approval', ?, ?, ?, ?, ?, ?)",
                               (user_id, wizard["title"], wizard["description"], wizard["link"], "[]", wizard["reward_amount"], wizard["workers"], budget, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), wizard["category"], wizard["task_type"], wizard["tutorial_image"], wizard["time_limit"], exp.strftime("%Y-%m-%d %H:%M:%S")))
                t_id = cursor.lastrowid
            
            admin_kbd = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Approve", callback_data=f"ad_task_app_{t_id}"), InlineKeyboardButton("❌ Reject", callback_data=f"ad_task_rej_{t_id}")]])
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"⚙️ **New Task #{t_id} Pending Approval!**", reply_markup=admin_kbd)
            safe_clear_state(context)
            await show_tasks_menu(update, "⏳ Your task has been submitted for admin review.")
        except Exception as e: logger.error(f"Publish Err: {e}")
    elif data == "wz_act_cancel_wizard_pipeline":
        safe_clear_state(context)
        await query.message.delete(); await show_main_menu(update, "❌ Cancelled.")
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dep_step == "ATTACHING_VERIFICATION_SCREENSHOT":
        ...
        try:
            if update.message.document: await context.bot.send_document(chat_id=ADMIN_ID, document=file_id, caption=f"📥 **New Deposit Request #{dep_id}**", parse_mode="Markdown")
            else: await context.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=f"📥 **New Deposit Request #{dep_id}**", parse_mode="Markdown")
        except: pass
        return
    ...
    if ticket_step == "ENTERING_TICKET_NET_MESSAGE":
        ...
        with db_transaction() as conn:
            ...
            t_id = cursor.lastrowid
            cursor.execute("INSERT INTO support_messages (ticket_id, sender_id, message_text, attachment_file_id, created_at) VALUES (?, ?, ?, ?, ?)", (t_id, user_id, update.message.caption or "Media Attached", file_id, now_str))
        await update.message.reply_text(f"✅ আপনার টিকিট সাবমিট হয়েছে। ID: #{t_id}", reply_markup=ReplyKeyboardMarkup([["🔙 Back"]], resize_keyboard=True))
        admin_kbd = InlineKeyboardMarkup([[InlineKeyboardButton("💬 Reply", callback_data=f"ad_supp_reply_{t_id}")]])
        try: await context.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=f"📞 Ticket #{t_id} Submitted.", reply_markup=admin_kbd)
        except: pass
        safe_clear_state(context)
        return
...
# =====================================================================
def main():
    if not TOKEN: raise ValueError("TELEGRAM_BOT_TOKEN error!")
    init_db()
    ...
    app.add_handler(CallbackQueryHandler(handle_admin_sub_nav, pattern=r'^ad_(fn|set|adm|task|sp)_'))
    app.add_handler(CallbackQueryHandler(handle_wizard_callbacks, pattern=r'^wz_'))
    app.add_handler(CallbackQueryHandler(admin_fin_chat_sub_callbacks, pattern=r'^(dp_g_mth_|supp_|ad_chat_|ad_tk_|ad_bal_|ad_supp_reply_|own_sub_)'))
    ...
    while True:
        try: app.run_polling(drop_pending_updates=True, close_loop=False)
