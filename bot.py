...
async def show_deposits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        if update.callback_query:
            await update.callback_query.message.reply_text("📥 কোনো পেন্ডিং ডিপোজিট রিকোয়েস্ট নেই।")
        else:
            await update.message.reply_text("📥 কোনো পেন্ডিং ডিপোজিট রিকোয়েস্ট নেই।")
        return
    reply_action = update.callback_query.message.reply_text if update.callback_query else update.message.reply_text
    await reply_action(f"📊 সর্বমোট {len(rows)} টি ডিপোজিট টিকিট পেন্ডিং আছে:")
    for r in rows:
...
async def show_pending_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, creator_id, title, reward_amount, total_slots, status FROM tasks WHERE status='pending_approval' ORDER BY id ASC")
        rows = cursor.fetchall()
    if not rows:
        if update.callback_query:
            await update.callback_query.message.reply_text("⏳ কোনো পেন্ডিং টাস্ক নেই।")
        else:
            await update.message.reply_text("⏳ কোনো পেন্ডিং টাস্ক নেই।")
        return
    text = "⏳ **পেন্ডিং টাস্ক তালিকা:**\n\n"
    for r in rows:
        text += f"ID: #{r[0]} | Creator: {r[1]}\n📌 {r[2]}\n💰 {r[3]} Tk x {r[4]} slots\n✅ এপ্রুভ: /approve_task {r[0]}\n------------------------\n"
    if update.callback_query:
...
async def approve_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with db_transaction() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT status, creator_id FROM tasks WHERE id = ?", (task_id,))
            row = cursor.fetchone()
            if not row or row[0] != 'pending_approval':
                await update.message.reply_text("❌ টাস্ক পাওয়া যায়নি বা ইতিমধ্যে এপ্রুভড।")
                return
            cursor.execute("UPDATE tasks SET status = 'active' WHERE id = ?", (task_id,))
        await update.message.reply_text(f"✅ টাস্ক #{task_id} এপ্রুভ করা হয়েছে।")
        try:
            await context.bot.send_message(chat_id=row[1], text=f"✅ আপনার টাস্ক #{task_id} এপ্রুভ করা হয়েছে এবং লাইভ হয়েছে!")
        except Exception: pass
    except ValueError:
        pass
...
async def show_withdraw_amounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💸 উইথড্র করার পরিমাণ নির্বাচন করুন:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    current_step = withdraw_state
    dep_step = deposit_state
    task_step = None 
    sub_step = submission_state
    review_step = None
    ...
    if current_step == "step_1":
        ...
        try:
            with db_transaction() as conn:
                ...
                cursor.execute("INSERT INTO withdrawals (user_id, amount, method, number, status, created_at) VALUES (?, ?, ?, ?, 'pending', ?)", (user_id, amount, method, number, 'pending', now_str))
...
# =====================================================================
# MACRO HANDLERS ARCHITECTURE (FIXES CRITICAL NAME-ERRORS/STARTUP COLLISIONS)
# =====================================================================
async def handle_force_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            await query.answer("❌ আপনি এখনও জয়েন করেননি! দয়া করে প্রথমে জয়েন করুন।", show_alert=True)
    except Exception:
        context.user_data["is_verified_session"] = True
        await query.message.delete()
        await process_welcome_access(update, context)
...
async def handle_wizard_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wz_cat_"):
        ...
        await context.bot.send_message(chat_id=user_id, text="📷 **ধাপ ৬**: কাজ বোঝানোর জন্য ছবি থাকলে আপলোড করুন (সর্বোচ্চ 5MB)। না থাকলে সরাসরি '⏭ Skip' চাপুন:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
...
async def handle_admin_review_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Fallback router aliasing admin commands safely directly into finance mapper filters rules."""
    await handle_browse_finances_and_tickets_callbacks(update, context)

async def handle_redirect_job_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer("Feature processing...", show_alert=False)
...
async def handle_regex_routing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text.startswith("/manage_task_"): await manage_task_handler(update, context)
    elif text.startswith("/view_sub_"): await view_submission_handler(update, context)
    elif text.startswith("/approve_sub_"): await approve_submission_cmd(update, context)
    elif text.startswith("/reject_sub_"): await reject_submission_cmd(update, context)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dep_step == "ATTACHING_VERIFICATION_SCREENSHOT":
        ...
        method_name = context.user_data.get("dep_method_name")
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with db_transaction() as conn:
            conn.execute("INSERT INTO deposits (user_id, amount, method_name, transaction_id, screenshot_file_id, status, created_at) VALUES (?, ?, ?, ?, ?, 'pending', ?)",
                         (user_id, amount, method_name, txid, photo_file_id, now_str))
        context.user_data.clear()
        await show_main_menu(update, "✅ আপনার ডিপোজিট রিকোয়েস্ট সাবমিট হয়েছে। অ্যাডমিন ভেরিফাই করবে।")
        return
    ...
    if sub_step == "uploading_photo_attachment_proof":
        ...
        try:
            with db_transaction() as conn:
                conn.execute("INSERT INTO task_submissions (task_id, worker_id, proof_text, proof_screenshot, status, created_at) VALUES (?, ?, ?, ?, 'pending', ?)",
            ...
            context.user_data.clear()
            await show_main_menu(update, "✅ আপনার কাজের প্রুফ সফলভাবে জমা দেওয়া হয়েছে!")
        except sqlite3.IntegrityError:
            await update.message.reply_text("❌ আপনি ইতিমধ্যেই এই টাস্কটি সাবমিট করেছেন।")
        return
    ...
    if ticket_step == "ENTERING_TICKET_NET_MESSAGE":
        ...
        with db_transaction() as conn:
            ...
            cursor.execute("INSERT INTO support_tickets (user_id, subject, created_at) VALUES (?, ?, ?)", (user_id, subj, now_str))
            t_id = cursor.lastrowid
            cursor.execute("INSERT INTO support_messages (ticket_id, sender_id, message_text, attachment_file_id, created_at) VALUES (?, ?, ?, ?, ?)", (t_id, user_id, update.message.caption or "Photo attached", photo_file_id, now_str))
        context.user_data.clear()
        await show_tasks_menu(update, f"✅ আপনার সাপোর্ট টিকিট তৈরি হয়েছে। টিকিট আইডি: #{t_id}")
        return
    ...
    if wizard_step == "TUTORIAL_IMAGE_UPLOAD":
        ...
        await update.message.reply_text("👥 **ধাপ ७**: এই কাজটি সর্বমোট কতজন ওয়ার্কারকে দিয়ে করাতে চান তার সংখ্যা টাইপ করে লিখে পাঠান (নূন্যতম ১০ জন):")
        return

# =====================================================================
# SYSTEM EXECUTIONS BOOTSTRAPPER CONFIGURATIONS
# =====================================================================
def main():
    if not TOKEN:
    ...
    # Handlers dynamic binding registrations
    ...
    # FIX: Callback names matching maps validated completely to block NameError loops
