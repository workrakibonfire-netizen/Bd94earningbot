...
def init_db():
    with sqlite3.connect(DATABASE) as conn:
        ...
        # Extension Tables for Safe Finalization & Warning Levels
        cursor.execute("""
...
# =====================================================================
# FORCE JOIN SECURE MEMBERSHIP PIPELINE (UPDATED MULTI-CHANNEL DETECTOR)
# =====================================================================
async def check_membership_status(bot, user_id) -> bool:
    if is_super_admin(user_id): return True
    
    # 1st Required Channel Check
    ...
    # 2nd Required Channel Mandated Addon
    ...
    try:
        ...
        logger.error(f"Force Join Bad Request: {e}. Channel check mismatch.")
...
def get_force_join_keyboard():
    ...
    if channel_val.startswith("@"):
        url1 = f"https://t.me/{channel_val[1:]}"
    elif channel_val.startswith("http"):
        url1 = channel_val
    else:
        url1 = get_setting("force_join_link", "https://t.me/bd94earning")

    url2 = "https://t.me/bd94earnings"

    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 Join Channel 1", url=url1)],
        [InlineKeyboardButton("📢 Join Channel 2 (Earnings)", url=url2)],
...
async def view_task_by_id_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, task_id: int):
    """Step-by-step navigation bootstrapper showing ALL required parameters cleanly."""
    user_id = update.effective_user.id
    try:
        with sqlite3.connect(DATABASE) as conn:
            task = conn.execute(
        ...
        out_msg = (
            ...
            f"📝 **Description:**\n{description}\n\n"
            f"🔗 **Target URL:** {link}\n\n"
            f"📋 **Proof Requirements Configuration:**\n{proof_reqs}"
        )
        
        kbd = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Next", callback_data=f"task_step1_next_{t_id}")],
...
# =====================================================================
# FORCE JOIN CALLBACK RESOLUTION GATE
# =====================================================================
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- STEP-BY-STEP PROOF CAPTURE FLOW ---
    if submission_state == "entering_proof_text_fields":
        if len(text) < 2: return await update.message.reply_text("⚠️ Proof text too short. Enter cleanly:")
        context.user_data["worker_submitted_proof_text"] = text
        t_id = context.user_data.get("sub_task_id")
        
        context.user_data["task_submission_step"] = "awaiting_proof_step2_photo"
        kbd = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Next", callback_data=f"task_step2_next_{t_id}")]])
        await update.message.reply_text("✅ Proof Text captured safely. Click 'Next' to initialize attachment window.", reply_markup=kbd)
        return
...
async def handle_admin_sub_nav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ad_fn_p_w":
        ...
        await query.message.reply_text("Send new Daily Bonus Amount:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="ad_nav_settings")]]))
    elif data == "ad_set_ref":
        context.user_data["admin_context_state"] = {"action": "ADMIN_EDIT_SETTING", "setting_key": "ref_reward"}
        await query.message.reply_text("Send new Referral Reward Amount:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="ad_nav_settings")]]))
    elif data == "ad_set_minw":
        context.user_data["admin_context_state"] = {"action": "ADMIN_EDIT_SETTING", "setting_key": "min_withdraw"}
        await query.message.reply_text("Send new Minimum Withdraw Amount:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="ad_nav_settings")]]))
    elif data == "ad_set_fj":
        context.user_data["admin_context_state"] = {"action": "ADMIN_EDIT_SETTING", "setting_key": "force_join_chat"}
        await query.message.reply_text("Send new Force Join Channel (e.g. @bd94earning or link):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="ad_nav_settings")]]))
    
    elif data == "ad_adm_add":
        context.user_data["admin_context_state"] = {"action": "ADMIN_ADD_ADMIN"}
        await query.message.reply_text("Send User ID to add as Sub Admin:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="ad_nav_admins")]]))
    elif data == "ad_adm_rem":
        context.user_data["admin_context_state"] = {"action": "ADMIN_REM_ADMIN"}
        await query.message.reply_text("Send User ID to remove Sub Admin privilege:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="ad_nav_admins")]]))
        ...
        broadcast_msg = (
            ...
            f"💎 **Reward:** {reward} Tk\n"
            f"👥 **Slots:** {workers}\n\n"
            f"🚀 **Start Working Now** → Find Job"
        )
        def run_category_alert(bot_instance, alert_text):
            try:
        ...
        threading.Thread(target=run_category_alert, args=(context.bot, broadcast_msg), daemon=True).start()
        ...
        try:
            with db_transaction() as conn:
                ...
                if not sub_row or sub_row[2] != 'pending':
                    return await query.message.reply_text("❌ Already finalized or mismatch occurred.")
                
                t_id, w_id, _, reward, creator_id = sub_row
                ...
                conn.execute("INSERT OR REPLACE INTO submission_reviews (submission_id, finalized_by, finalized_time, history_logs) VALUES (?, 'Approved System', ?, 'Submitted -> Approved')", (s_id, now_str))
            ...
            try: 
                app_notif = f"✅ **Task Approved**\n\n💰 Reward Added Successfully: {reward} Tk"
                await context.bot.send_message(chat_id=w_id, text=app_notif)
            except: pass
        except Exception as e: logger.error(f"Review approve exception trace: {e}")

    elif data.startswith("own_sub_rej_"):
        s_id = int(data.split("_")[-1])
        context.user_data["admin_context_state"] = {"action": "OWNER_MANDATORY_REJECT_REASON", "sub_id": s_id}
        await query.message.reply_text("❌ Please send the rejection reason via text input below:")
...
async def handle_browse_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("task_step1_next_"):
        t_id = int(data.split("_")[-1])
        context.user_data["sub_task_id"] = t_id
        context.user_data["task_submission_step"] = "entering_proof_text_fields"
        await query.message.reply_text("📝 **Step 2:** Please enter required Text Proof inside message bar below now:")
        try: await query.message.delete()
        except: pass
        return

    elif data.startswith("task_step2_next_"):
        t_id = int(data.split("_")[-1])
        await query.message.reply_text("📸 **Step 3:** Please upload requested Screenshot image proof configuration now (or type 'Skip' to finalize):")
        try: await query.message.delete()
        except: pass
        return
    ...
    if data.startswith("br_start_task_"):
        t_id = int(data.split("_")[-1])
        await view_task_by_id_handler(update, context, t_id)
        return
...
async def approve_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for (uid,) in all_users:
            try: await context.bot.send_message(chat_id=uid, text=broadcast_msg)
            except: pass
    except: pass
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub_step == "awaiting_proof_step2_photo":
        ...
        try:
            with db_transaction() as conn:
                ...
                if not task_data:
                    return await update.message.reply_text("❌ Target task profile matching error.")
                
                creator_id, t_title, reward = task_data
            ...
            try:
                ...
                owner_alert = (
                    ...
                    f"📋 **Task:** {t_title}\n"
                    f"💰 **Reward:** {reward} Tk\n\n"
                    f"Please review via `/manage_task_{task_id}`"
                )
                await context.bot.send_message(chat_id=creator_id, text=owner_alert)
            except: pass

            safe_clear_state(context)
            await show_main_menu(update, "✅ Proof Submitted successfully! Pending balance updated.")
        except Exception as e: 
            logger.error(f"Proof submission compilation fault: {e}")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_browse_callbacks, pattern=r'^(br_|browse_|task_step)')) 
