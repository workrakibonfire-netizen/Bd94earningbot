...
# =====================================================================
# SYSTEM TASK PRICING CONFIGURATION
# =====================================================================
MIN_RATES = {
    "Telegram Join": {"Join Channel/Group": 3},
    "Telegram Views": {"Post View Flow": 2},
    "YouTube Comment": {"Video Interaction": 2},
    "TikTok Watch+Like+Comment+Share": {"Full Optimization": 3},
    "Gmail New": {"Fresh Registration": 8},
    "Gmail Old": {"Aged Account Verification": 8},
    "Mobile Apps": {"Download & Install": 10}
}

# Remap human readable display categories
CAT_MAP = {
    "Telegram Join": "📂 Telegram Join",
    "Telegram Views": "📂 Telegram Views",
    "YouTube Comment": "📂 YouTube Comment",
    "TikTok Watch+Like+Comment+Share": "📂 TikTok Watch+Like+Comment+Share",
    "Gmail New": "📂 Gmail New",
    "Gmail Old": "📂 Gmail Old",
...
# =====================================================================
# BACKGROUND AUTO EXPIRATION ENGINE
# =====================================================================
def check_and_expire_tasks(conn=None):
    ...
    try:
        ...
        logger.error(f"Error inside task expiration loops: {e}")
    finally:
        if should_close:
            conn.commit()
            conn.close()
...
def run_web_server():
    class HealthHandler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"OK - BD94 Core Architecture Active")
    ...
    try:
        with socketserver.TCPServer(("0.0.0.0", port), HealthHandler) as server:
            logger.info(f"Keep-alive web server initialized over port {port}")
            server.serve_forever()
    except Exception as e:
        logger.error(f"Keep-Alive port-binding allocation error: {e}")

# =====================================================================
# USER NETWORK SPECIFICATIONS LOGIC
# =====================================================================
...
# =====================================================================
# CORE IMPLEMENTATIONS INTERFACES TELEGRAM
# =====================================================================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID and not context.user_data.get("is_verified"):
        try:
            ...
            logger.warning(f"Force Join interface connection validation failure: {e}")
            await update.message.reply_text("⚠️ ভেরিফিকেশন সিস্টেমে সাময়িক কারিগরি সমস্যা হচ্ছে। অনুগ্রহ করে কিছুক্ষণ পর চেষ্টা করুন।")
            return
...
# =====================================================================
# MANAGEMENT CHANNELS OPERATIONAL TIER (ADMIN INTERACTIVE COMMANDS)
# =====================================================================
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🛠️ Employer Task Moderation Rules:\n"
        "👉 /show_pending_tasks - Display all tasks awaiting admin validation activation\n"
        "👉 /approve_task <id> - Validate and broadcast new employer campaign"
    )
...
# =====================================================================
# MODERATIONS TRANSACTIONS CAPABILITIES (ADMIN PANEL PAYOUT REGION)
# =====================================================================
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=f"✅ উইথড্রাল রিকোয়েস্ট অ্যাপ্রুভড!\n\n canালানো আইডি: #{req_id} নম্বর উইথড্রাল রিকোয়েস্টটি সফলভাবে সম্পন্ন হয়েছে।\n💰 পরিমাণ: {amount} Tk\n📱 মেথড: {method}"
            )
        except Exception:
            pass
        ...
        await update.message.reply_text("❌ Payout allocation process failure.")
...
# =====================================================================
# PAYMENT SETTING REGION ARCHITECTURE
# =====================================================================
...
# =====================================================================
# AUDITED AUDIT DISPATCH INBOUND DEPOSITS ENGINE 
# =====================================================================
...
async def approve_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=f"✅ ডিপোজিট সফলভাবে অ্যাপ্রুভ করা হয়েছে!\n\nডিপোজিট আইডি: #{dep_id}\n💰 পরিমাণ: {amount} Tk আপনার Deposit Balance এ যুক্ত করা হয়েছে যা দিয়ে আপনি কাজ পোস্ট করতে পারবেন।"
            )
        except Exception:
            pass
...
# =====================================================================
# SYSTEM CORE MESSAGE TEXT PATTERNS ROUTER REGEX
# =====================================================================
...
# =====================================================================
# EMPLOYER TASK VALIDATION CONTROLLERS (ADMIN MODERATION FLOW)
# =====================================================================
async def show_pending_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, creator_id, title, category, reward_amount FROM tasks WHERE status = 'pending_approval' ORDER BY id ASC")
        rows = cursor.fetchall()
    if not rows:
        await update.message.reply_text("📥 কোনো টাস্ক বর্তমানে অ্যাডমিন এপ্রুভালের জন্য পেন্ডিং নেই।")
        return
    text = "⏳ **Pending Tasks Awaiting Admin Verification:**\n\n"
    for r in rows:
        text += f"Task ID: #{r[0]} | Creator: {r[1]}\n📌 Title: {r[2]}\n📂 Cat: {r[3]} | Reward: {r[4]} Tk\n👉 Validate & Broadcast: /approve_task {r[0]}\n------------------------\n"
    await update.message.reply_text(text)
...
async def approve_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    if not context.args:
        await update.message.reply_text("নিয়ম: /approve_task <task_id>")
        return
    try:
    ...
    try:
        with db_transaction() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT creator_id, category, reward_amount, status FROM tasks WHERE id = ?", (task_id,))
            row = cursor.fetchone()
            if not row:
                await update.message.reply_text("❌ এই টাস্ক আইডিটি সিস্টেমে পাওয়া যায়নি।")
                return
            creator_id, category, reward, status = row
            if status != "pending_approval":
                await update.message.reply_text(f"❌ টাস্কটি ইতিমধ্যেই সম্পন্ন বা সক্রিয় অবস্থায় রয়েছে! স্ট্যাটাস: {status}")
                return
        ...
        await update.message.reply_text(f"✅ Task #{task_id} সফলভাবে সচল করা হয়েছে! গ্লোবাল ইউজার নোটিফিকেশন ব্রডকাস্ট শুরু হচ্ছে...")
        
        # Broadcast Campaign Alert directly to users globally
        broadcast_msg = (
            f"🔥 **নতুন কাজ এসেছে!**\n\n"
            f"📂 Category: {category}\n"
            f"💰 Reward: ৳{reward}\n"
            f"⏰ Valid: 3 Days\n\n"
            f"👇 এখনই কাজ শুরু করতে নিচের বাটনে চাপ দিন"
        )
        ...
        for uid in all_users:
            try:
                await context.bot.send_message(chat_id=uid, text=broadcast_msg, reply_markup=keyboard, parse_mode="Markdown")
                sent_cnt += 1
                await asyncio.sleep(0.04)
            except Exception:
                pass
        logger.info(f"Broadcast campaign completed for task #{task_id} across {sent_cnt} profiles.")
    except Exception as e:
        logger.error(f"Error inside approve task routing: {e}")

# =====================================================================
# REDESIGN: 🔎 FIND JOB BROWSER CATEGORIES CONTROLLERS WITH COUNTS 
# =====================================================================
async def find_jobs_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    summary_text = "🔎 **সব উপলব্ধ কাজসমূহ (Find Job Portal)**\n\n"
    keyboard = []
    row_btn = []
    
    for cat_name, display_label in CAT_MAP.items():
        cnt = counts.get(cat_name, 0)
        summary_text += f"{display_label} ({cnt})\n"
        row_btn.append(InlineKeyboardButton(cat_name.split()[0], callback_data=f"browse_cat_{cat_name}"))
        if len(row_btn) == 2:
    ...
    summary_text += "\n👇 নিচের বাটন থেকে আপনার পছন্দের ক্যাটাগরি বেছে নিন:"
...
# =====================================================================
# BANGLA INLINE DRIVEN WIZARD PROCESSING LOGIC (EMPLOYER PORTAL)
# =====================================================================
async def start_job_wizard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    row = []
    for cat in MIN_RATES.keys():
        row.append(InlineKeyboardButton(cat, callback_data=f"wiz_cat_{cat[:15]}"))
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
    ...
    await update.message.reply_text("💼 **নতুন কাজ পোস্ট করার উইজার্ড (Bangla Form System)**\n\n👇 প্রথমে নিচের অপশন থেকে আপনার কাজের মূল ক্যাটাগরি সিলেক্ট করুন:", reply_markup=reply_markup)
...
async def handle_wizard_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wiz_cat_"):
        cat_prefix = data.replace("wiz_cat_", "").strip()
        selected_cat = None
        for k in MIN_RATES.keys():
        ...
        keyboard = [[InlineKeyboardButton(f"🔗 {t}", callback_data=f"wiz_type_{t[:20]}")] for t in types]
        ...
        await context.bot.send_message(chat_id=user_id, text="📷 **ধাপ ৬**: ওয়ার্কারদের কাজ বোঝানোর সুবিধার্থে কোনো টিউটোরিয়াল ছবি থাকলে তা আপলোড করুন। না থাকলে সরাসরি '⏭ Skip' বাটন চাপুন:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
        ...
        default_rate = MIN_RATES.get(cat, {}).get(t_type, 3)
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Submit for Approval", callback_data="wiz_action_publish")],
        ...
        try:
            ...
            context.user_data.clear()
            await show_tasks_menu(update, f"🎉 **আপনার টাস্কটি অ্যাডমিন রিভিউর জন্য সফলভাবে সাবমিট করা হয়েছে!**\n\n🆔 টাস্ক আইডি: #{inserted_task_id}\nঅ্যাডমিন এটি রিভিউ করে অনুমোদন দিলে এটি লাইভ ব্রডকাস্ট হবে এবং মেম্বাররা কাজ করতে পারবে।")
            
            # Send dynamic confirmation link context alert into Admin Channel instantly
            try:
                await context.bot.send_message(
                    chat_id=ADMIN_ID,
...
# =====================================================================
# RE-REFACTORED INTEGRATED TASK REVIEW NOTIFICATION ENGINE CALLBACKS
# =====================================================================
async def handle_admin_review_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID:
        await query.answer("❌ Unauthorized Access Blocked!", show_alert=True)
        return
        
    await query.answer()
    
    # --- View Proof Interactive Callback Subsystem ---
    if data.startswith("admin_view_sub_"):
        sub_id = int(data.replace("admin_view_sub_", "").strip())
        with sqlite3.connect(DATABASE) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT s.proof_text, s.proof_screenshot, s.worker_id, s.task_id, t.title 
                FROM task_submissions s JOIN tasks t ON s.task_id = t.id WHERE s.id = ?
            """, (sub_id,))
            row = cursor.fetchone()
            
        if not row:
            await query.message.reply_text("❌ সাবমিশন রেকর্ড হিস্ট্রি পাওয়া যায়নি।")
            return
            
        proof_text, screenshot, worker_id, task_id, title = row
        out = f"👁️ **Submissions Verification Context Assets (Sub #{sub_id})**\n\n" \
              f"📌 Task ID: #{task_id} - {title}\n" \
              f"👤 Worker Profile ID: {worker_id}\n\n" \
              f"💬 Written Text Proof:\n{proof_text}\n"
              
        if screenshot:
            try:
                await context.bot.send_photo(chat_id=ADMIN_ID, photo=screenshot, caption=out)
                return
            except Exception: pass
        await query.message.reply_text(out)
        return

    # --- Approve Flow Pipeline Subsystem ---
    elif data.startswith("admin_app_sub_"):
        sub_id = int(data.replace("admin_app_sub_", "").strip())
        try:
            with db_transaction() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT s.task_id, s.worker_id, s.status, t.reward_amount 
                    FROM task_submissions s JOIN tasks t ON s.task_id = t.id WHERE s.id = ?
                """, (sub_id,))
                row = cursor.fetchone()
                if not row:
                    await query.message.reply_text("❌ সাবমিশন ডাটা খুঁজে পাওয়া যায়নি।")
                    return
                t_id, worker_id, status, reward = row
                if status != "pending":
                    await query.message.reply_text(f"❌ এই সাবমিশনটি ইতিমধ্যে রিভিও করা হয়েছে! স্ট্যাটাস: {status}")
                    return
                ...
                """, (worker_id, reward, f"Sub-{sub_id}", created_at))
                
            await query.message.edit_reply_markup(reply_markup=None)
            await query.message.reply_text(f"✅ Submission #{sub_id} successfully APPROVED. Reward sent to worker.")
            
            try:
                await context.bot.send_message(
                    chat_id=worker_id,
                    text=f"🎉 **অভিনন্দন!**\n\nআপনার টাস্ক সাবমিশনটি সফলভাবে Approved হয়েছে।\n💰 Reward Added: ৳{reward}"
                )
            except Exception: pass
        except Exception as e:
            logger.error(f"Error executing admin approval workflow query lines: {e}")
        return

    # --- Reject Trigger Flow Subsystem ---
    elif data.startswith("admin_rej_sub_"):
        sub_id = int(data.replace("admin_rej_sub_", "").strip())
        context.user_data.clear()
        context.user_data["admin_reject_reason_sub_id"] = sub_id
        await query.message.reply_text(f"❌ Submission #{sub_id} বাতিল করার উইজার্ড।\n\nওয়ার্কার কি কারণে কাজটি ভুল করেছে তার স্পষ্ট একটি কারণ (Rejection Reason) এখানে লিখে রিপ্লাই পাঠান:")
        return

# =====================================================================
# BROADCAST ROUTED REDIRECT CLICK INTERCEPT PASS FILTER
# =====================================================================
async def handle_redirect_job_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    task_id = query.data.replace("redirect_job_", "").strip()
    
    # Pass virtual simulated string payload directly into regex handlers parsing
    update.message = query.message
    update.message.text = f"/view_task_{task_id}"
    await view_task_handler(update, context)
...
async def handle_force_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning(f"Force join checkpoint warning log statement: {e}")
        bot_is_admin = False
...
# =====================================================================
# CORE SYSTEM MESSAGE DIALOG CONTROLLER & STATE MACHINES INTERFACES 
# =====================================================================
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_rej_sub_id = context.user_data.get("admin_reject_reason_sub_id")
    ...
    # --- Phase 2 Admin Rejection Text Collector Capture Step ---
    if admin_rej_sub_id:
        reason = text.strip()
        if len(reason) < 3:
            await update.message.reply_text("⚠️ অনুগ্রহ করে স্পষ্ট বাতিলের কারণ টাইপ করে পাঠান:")
            return
        ...
        try:
            with db_transaction() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT s.task_id, s.worker_id, s.status 
                    FROM task_submissions s WHERE s.id = ?
                """, (admin_rej_sub_id,))
                row = cursor.fetchone()
                if not row or row[2] != "pending":
                    await update.message.reply_text("❌ সাবমিশন স্ট্যাটাস ইতিমধ্যে পরিবর্তিত হয়েছে।")
                    context.user_data.clear()
                    return
                t_id, worker_id, _ = row
                cursor.execute("UPDATE task_submissions SET status = 'rejected', admin_note = ? WHERE id = ?", (reason, admin_rej_sub_id))
                cursor.execute("UPDATE tasks SET filled_slots = CASE WHEN filled_slots > 0 THEN filled_slots - 1 ELSE 0 END WHERE id = ?", (t_id,))
                
            context.user_data.clear()
            await update.message.reply_text(f"❌ Submission #{admin_rej_sub_id} marked as REJECTED successfully.")
            ...
            logger.error(f"Error inside text capture rejection pathway logic: {e}")
        return
    ...
    if wizard_step:
        ...
        if wizard_step == "TITLE":
            ...
            wizard["step"] = "PROOF_NAME"
            await update.message.reply_text("📋 **ধাপ ৫**: কাজের প্রমাণ যাচাইয়ের জন্য প্রুফ সেটআপ।\n\n👇 ১ম প্রুফের নাম কি হবে তা লিখে পাঠান (যেমন: Username, Screenshot):")
            return
    ...
    if text == "📊 Dashboard":
        ...
        with sqlite3.connect(DATABASE) as conn:
            ...
            row = cursor.fetchone()
        await show_wallet_menu(update, f"💳 **सिक्योर ওয়ালেট সেন্টার**\n\n📥 ডিপোজিট ব্যালেন্স: {row[0] if row else 0} Tk\n💰 আর্নিংস ব্যালেন্স: {row[1] if row else 0} Tk")
...
# =====================================================================
# SECURE ATOMIC TASK PROOF SUBMISSIONS INBOUND 
# =====================================================================
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wizard_step == "TUTORIAL_IMAGE":
        ...
        try:
            ...
            logger.info(f"User {user_id} added deposit ticket #{dep_id}. Amount: {amount} Tk. Unique TxID: {txid}")
            context.user_data.clear()
            await show_main_menu(update, f"✅ আপনার ডিপোজিট রিকোয়েস্টটি ম্যানুয়াল ভেরিফিকেশনের জন্য অ্যাডমিনের কাছে পাঠানো হয়েছে। টিকিট আইডি: #{dep_id}")
            
            admin_caption = (
                f"📥 🆕 New Deposit Submitted!\n\n"
                f"Deposit ID: #{dep_id}\n"
                f"User ID: {user_id}\n"
                f"Amount: {amount} Tk\n"
                f"Method: {m_name}\n"
                f"TxID: {txid}\n"
                f"Date Logs: {created_at}\n\n"
            ...
            await update.message.reply_text("❌ Security Block: এই ট্রানজেকশন আইডি (TxID) ইতিমধ্যে বটের ডাটাবেসে ব্যবহার করা হয়েছে। ডুপ্লিকেট সাবমিশন গ্রহণযোগ্য নয়।")
            context.user_data.clear()
        return

    # --- Worker Work Verification Submission Subsystem ---
        ...
        try:
            with db_transaction() as conn:
                ...
                if not task_data or task_data[3] != "active" or task_data[2] >= task_data[1]:
                    await show_tasks_menu(update, "❌ দুঃখিত! এই কাজটির স্লট পূর্ণ হয়ে গেছে অথবা টাস্কটি এক্সপায়ার হয়ে গেছে।")
                    context.user_data.clear()
                    return
            ...
            # Formulate Bangla UI State Notification 
            await show_tasks_menu(
                update, 
                "⏳ আপনার কাজ রিভিউয়ের জন্য পাঠানো হয়েছে।\n\n"
                "📋 Status: Pending Review"
            )
            
            # Send Comprehensive Review Alert directly to Admin Panel via Inline Controls
            ...
            review_keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Approve", callback_data=f"admin_app_sub_{sub_id}"),
                    InlineKeyboardButton("❌ Reject", callback_data=f"admin_rej_sub_{sub_id}")
                ],
                [InlineKeyboardButton("👁 View Proof", callback_data=f"admin_view_sub_{sub_id}")]
            ])
            try:
...
# =====================================================================
# INTERCEPT ROUTER REGULAR CORE ENGINE ENTRYBOOTSTRAPPER
# =====================================================================
def main():
    if not TOKEN:
    ...
    # Custom newly added employer task activation commands
    ...
    # Specific verification check mapping handlers
    ...
    app.add_handler(CallbackQueryHandler(handle_admin_review_callbacks, pattern=r'^admin_'))
    ...
    print("BD94 Dynamic Marketplace Infrastructure Active [Task Review Engine Online]...")
    app.run_polling()
