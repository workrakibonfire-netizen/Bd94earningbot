...
# =====================================================================
# FIXED/RESTORED ADMINISTRATIVE CAPABILITIES FUNCTIONS (CRITICAL REPAIR)
# =====================================================================
...
# =====================================================================
# CUSTOM INTERACTION HANDLERS (DYNAMIC TEXT INPUT FIELDS MACHINE)
# =====================================================================
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "MANDATORY_REJECT_REASON_SUBMISSION":
            ...
            try:
                with db_transaction() as conn:
                    ...
                    row = cursor.fetchone()
                    if not row or row[2] != 'pending':
                        await update.message.reply_text("❌ এই সাবমিশনটি ইতিমধ্যে প্রসেস করা হয়েছে বা সিস্টেমে নেই।")
                        context.user_data.clear()
                        return
            ...
            try:
                ...
                if usr:
                    try:
                        await context.bot.send_message(chat_id=usr[0], text=f"📨 **সাপোর্ট টিকিট লাইভ চ্যাট আপডেট!**\n\nআপনার ওপেন টিকিট #{t_id} এ সাপোর্ট টিম থেকে উত্তর এসেছে:\n\n💬 {text}")
                    except Exception: pass
            except Exception as e:
    ...
    if text == "📊 Dashboard":
        ...
        await update.message.reply_text(
            ...
            f"📥 Deposit Balance: {r[0] if r else 0} Tk\n"
            f"💰 Earning Balance: {r[1] if r else 0} Tk\n"
            f"⏳ Pending Balance: {r[2] if r else 0} Tk\n"
            f"👥 মোট সফল রেফারেল: {r[3] if r else 0} জন"
        )
        ...
        for r in rows:
            ...
            text_out += f"🆔 কাজ ম্যানেজ করতে ক্লিক: /manage_task_{r[0]}\n📌 টাইটেল: {r[1]}\n💰 ওয়ার্কার রিওয়ার্ড: {r[2]} Tk | প্রুফ জমা: {r[3]}/{r[4]} | স্ট্যাটাস: {status_lbl}\n------------------------\n"
        ...
        wallet_text = (
            ...
            f"📥 Deposit Balance: {r[0]} Tk\n"
            f"💰 Earning Balance: {r[1]} Tk\n"
        ...
        if balance < min_withdraw:
            await update.message.reply_text(f"❌ দুঃখিত, আপনার ব্যালেন্স পর্যাপ্ত নয়! নূন্যতম উইথড্রয়াল লিমিট {min_withdraw} Tk। আপনার ব্যালেন্স আছে: {balance} Tk")
            return
        ...
        try:
            with db_transaction() as conn:
                ...
                cursor.execute("UPDATE users SET earnings_balance = earnings_balance + ? WHERE user_id = ?", (daily_bonus_amt, user_id))
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wizard_step == "TUTORIAL_IMAGE_UPLOAD":
        ...
        await update.message.reply_text("📸 নিয়মের ছবি সংযুক্ত হয়েছে।\n\n👥 **ধাপ ৭**: মোট কতজন ওয়ার্কার প্রয়োজন তা লিখে পাঠান:")
        return
        ...
        with sqlite3.connect(DATABASE) as conn:
            ...
            if cursor.fetchone()[0] > 0:
                await update.message.reply_text("❌ এই স্ক্রিনশট ফাইলটি ইতিমধ্যে পেমেন্ট ভেরিফিকেশনে ব্যবহৃত হয়েছে।")
                return
        ...
        try:
            ...
            context.user_data.clear()
            await show_main_menu(update, f"✅ ডিপোজিট রিকোয়েস্ট ম্যানুয়াল ভেরিফিকেশনের জন্য সাবমিট করা হয়েছে। টিকিট আইডি: #{dep_id}")
            
            admin_caption = (
                f"📥 **নতুন ডিপোজিট রিকোয়েস্ট!**\n\n"
                f"Deposit ID: #{dep_id}\nUser ID: {user_id}\nAmount: {amount} Tk\nMethod: {m_name}\nTxID: {txid}\nDate: {now_str}\n\n"
                f"👉 Approve: /approve_deposit {dep_id}\n👉 Reject: /reject_deposit {dep_id} Reason"
            )
            try:
                await context.bot.send_photo(chat_id=ADMIN_ID, photo=photo_file_id, caption=admin_caption)
            except Exception: pass
        except sqlite3.IntegrityError:
            await update.message.reply_text("❌ এই ট্রানজেকশন আইডি (TxID) ইতিমধ্যে ডাটাবেসে ব্যবহার করা হয়েছে।")
            context.user_data.clear()
        return
        ...
        try:
            with db_transaction() as conn:
                ...
                if not task_data or task_data[3] != 'active' or task_data[2] >= task_data[1]:
                    await show_tasks_menu(update, "❌ টাস্কটি এক্সপায়ার হয়ে গেছে বা স্লট পূর্ণ হয়ে গেছে।")
                    context.user_data.clear()
                    return
            ...
            username_ref = f"@{update.effective_user.username}" if update.effective_user.username else f"User_{user_id}"
            admin_review_msg = (
                f"🔔 **নতুন Task Submission**\n\n👤 Worker User: {username_ref}\n🆔 Worker User ID: {user_id}\n\n"
                f"📂 Category: {task_data[4]}\n📋 Task Title: {task_data[5]}\n\n📎 Status: Pending Administrative Verification View"
            )
            review_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Approve", callback_data=f"ad_app_sub_{sub_id}"), InlineKeyboardButton("❌ Reject", callback_data=f"ad_rej_sub_{sub_id}")],
...
async def handle_wizard_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wz_cat_"):
        ...
        await context.bot.send_message(chat_id=user_id, text="📷 **ধাপ ৬**: কাজ বোঝানোর জন্য ছবি থাকলে আপলোড করুন। না থাকলে '⏭ Skip' চাপুন:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    elif data.startswith("wz_tm_"):
        days = data.replace("wz_tm_", "").strip()
        ...
        preview_msg = (
            f"📊 **কাজের চূড়ান্ত রিভিউ ও ইনভয়েস (Preview Dashboard)**\n\n📁 ক্যাটাগরি: {cat}\n🔗 ধরণ: {t_type}\n📌 শিরোনাম: {title}\n🔗 লিংক: {link}\n📄 বিবরণ: {desc}\n📋 প্রুফ রিকোয়ারমেন্টস:\n{proof_text_summary}👥 মোট ওয়ার্কার: {workers} জন\n💰 প্রতি ওয়ার্কার পেমেন্ট: {reward_amount} Tk\n📅 সময়সীমা: {days} দিন\n\n💵 কাজের মোট বাজেট: {subtotal} Tk\n⚡ প্ল্যাটফর্ম প্রসেসিং ফি: {fee} Tk\n💳 সর্বমোট প্রদেয় ব্যালেন্স: **{total_budget} Tk**\n\nটাস্কটি সাবমিট করতে নিচের বাটনে ক্লিক করুন। এটি অ্যাডমিন এপ্রুভ করার পর লাইভ ব্রডকাস্ট হবে।"
        )
        
        keyboard = [
            [InlineKeyboardButton("✅ Submit for Approval", callback_data="wz_act_publish")],
        ...
        try:
            with db_transaction() as conn:
                ...
                if dep_balance < budget:
                    await context.bot.send_message(chat_id=user_id, text=f"❌ **ডিপোজিট ব্যালেন্স পর্যাপ্ত নয়!**\n\nপ্রয়োজনীয় বাজেট: {budget} Tk\nআপনার ব্যালেন্স: {dep_balance} Tk")
                    context.user_data.clear()
                    return
...
async def handle_browse_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("browse_cat_"):
        ...
        for tk in tasks_list:
            ...
            text_out += f"🆔 কাজ ওপেন করুন: /view_task_{t_id}\n💎 শিরোনাম: **{title}**\n💰 রিওয়ার্ড: {reward} Tk | 👥 বাকি স্লট: {slots_left} জন\n📝 সংক্ষিপ্ত বিবরণ: {short_desc}\n------------------------\n"
    ...
    if data.startswith("dp_g_mth_"):
        ...
        if sect == "users":
            ...
            if not rows:
                ...
                return
            out = "📨 **오픈 সাপোর্ট টিকেট প্যানেল তালিকা:**\n\n"
            keyboard = []
            for r in rows:
            ...
            with sqlite3.connect(DATABASE) as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM tasks WHERE status='pending_approval'")
                p_tsk = cursor.fetchone()[0]
            msg = f"📊 **সিস্টেম অ্যানালিটিক্স ও ড্যাশবোর্ড**\n\n👥 সর্বমোট নিবন্ধিত ইউজার: {tot_u} জন\n📥 মোট এপ্রুভড ডিপোজিট: {tot_d} টি\n💸 মোট এপ্রুভড উইথড্রয়াল: {tot_w} টি\n⏳ অপেক্ষমান টাস্ক: {p_tsk} টি"
        ...
        if not items:
            ...
            return
        out = "🔔 **আপনার নোটিফিকেশন হিস্ট্রি লগ:**\n\n"
        for m, dt in items:
            out += f"📅 [{dt}]\n💬 {m}\n------------------------\n"
        ...
        except Exception as e: logger.error(f"Error handling task submission approval: {e}")
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"❌ Submission #{sub_id} রিজেক্ট করার উইজার্ড। বাতিলের কারণ টাইপ করে পাঠান:")
        return
...
async def handle_redirect_job_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    t_id = query.data.replace("redirect_job_", "").strip()
    update.message = query.message
    update.message.text = f"/view_task_{t_id}"
...
async def approve_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with db_transaction() as conn:
            ...
            if not row or row[2] != 'pending_approval':
                await update.message.reply_text("❌ এই টাস্কটি রিভিউ মোডে নেই বা পাওয়া যায়নি।")
                return
        ...
        broadcast_msg = f"🆕 **নতুন কাজ এসেছে মার্কেটপ্লেসে!**\n\n📂 ক্যাটাগরি: {row[0]}\n💰 পেমেন্ট রিওয়ার্ড: ৳{row[1]}\n⏰ সময়সীমা: ৩ দিন\n\n👇 এখনই কাজ শুরু করতে নিচের লিংকে চাপুন:"
    ...
    except Exception as e: logger.error(f"Error approving task: {e}")
