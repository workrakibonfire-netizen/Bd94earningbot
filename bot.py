...
def run_web_server():
    class HealthHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def log_message(self, format, *args):
            return # Suppress to prevent log overflow

    port = int(os.getenv("PORT", "3000"))
    socketserver.TCPServer.allow_reuse_address = True # CRITICAL for Render restarts
...
def db_transaction(retries=5, delay=0.5):
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            yield conn
            conn.commit()
            return  # Success, exit the context manager safely
...
async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Ignore normal network timeouts to prevent crash loops and user spam
        ...
        # Only notify the user if the update object exists and it's a real crash
...
async def check_membership_status(bot, user_id) -> bool:
    """Robustly checks if a user is in the required channel API-side."""
    if is_super_admin(user_id): return True
    channel_id = get_setting("force_join_chat", "@bd94earning")
    
    # Secure ID formatting for the API check
...
def get_force_join_keyboard():
    """Generates the inline keyboard, ensuring URLs are fully valid to prevent rendering crashes."""
...
async def complete_user_registration(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, referrer_id: int = 0):
    """Handles Database insertion ONLY after successful verification."""
    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # --- SEND REFERRAL NOTIFICATION TO USER A ---
    if is_new_user and referrer_id != 0:
        notification_text = (
            ...
            f"Withdrawal সম্পন্ন হলে আপনার Pending Bonus স্বয়ংক্রিয়ভাবে Successful Bonus হয়ে যাবে এবং {ref_reward} টাকা আপনার Wallet Balance-এ যোগ হবে।\n\n"
        ...
        try:
            ...
            logger.warning(f"Referrer {referrer_id} blocked the bot. Notification safely skipped.")
        except telegram.error.BadRequest as e:
            logger.warning(f"Bad Request for referrer {referrer_id}: {e}")
        except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_joined:
        ...
        markup = get_force_join_keyboard()
        await update.message.reply_text(warn, reply_markup=markup)
        return # HALT EXECUTIONS: DO NOT INSERT INTO DB YET
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔍 Find Job": await find_jobs_start(update, context)
        ...
        with sqlite3.connect(DATABASE) as conn:
            ...
            # Fetch Successful Referrals (Users who completed first withdrawal)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dep_step == "ATTACHING_VERIFICATION_SCREENSHOT":
        ...
        with db_transaction() as conn: 
            cursor = conn.cursor()
            cursor.execute("INSERT INTO deposits (user_id, amount, method_name, transaction_id, screenshot_file_id, status, created_at) VALUES (?, ?, ?, ?, ?, 'pending', ?)", (user_id, amount, m_name, txid, photo_file_id, now_str))
...
def main():
    ...
    try:
        ...
        logger.critical(f"Fatal application failure: {e}\n{traceback.format_exc()}")
