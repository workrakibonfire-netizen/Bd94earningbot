...
def init_db():
    with sqlite3.connect(DATABASE) as conn:
        ...
        # deposits table with screenshot_file_id migration audit check
        cursor.execute("""
        ...
        if "screenshot_file_id" not in dep_columns:
            ...
            logger.info("Database Migration: Added screenshot_file_id column to deposits table.")
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📥 /show_deposits - View all pending deposits with screenshot previews\n"
...
# =====================================================================
# AUDITED ADMIN DEPOSIT MANAGEMENT
# =====================================================================
async def show_deposits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📊 Total Pending Deposits Counter: {pending_count} (Showing Newest First)")
    
    for r in rows:
        caption_text = (
            f"📥 Pending Deposit Ticket\n\n"
            f"Deposit ID: #{r[0]}\n"
            f"User ID: {r[1]}\n"
            f"Amount: {r[2]} Tk\n"
            f"Method: {r[3]}\n"
        ...
        if r[7]:  # If screenshot_file_id exists
            try:
                await context.bot.send_photo(chat_id=ADMIN_ID, photo=r[7], caption=caption_text)
            except Exception as e:
                logger.error(f"Failed to display saved screenshot for deposit #{r[0]}: {e}")
                await update.message.reply_text(caption_text + "\n\n⚠️ Screenshot file reference corrupt or vanished from servers.")
        else:
            await update.message.reply_text(caption_text + "\n\n⚠️ Warning: No verification screenshot file recorded for this transaction.")
...
async def approve_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with db_transaction() as conn:
            ...
            user_id, amount, status = row
            if status != "pending":
                await update.message.reply_text(f"❌ Security Block: Deposit ticket already processed. Current Status: {status}.")
                return
            ...
            logger.info(f"Deposit #{dep_id} APPROVED. {amount} Tk added to user {user_id} deposit balance.")
        ...
        try:
            ...
            pass
        await update.message.reply_text(f"✅ Deposit Request #{dep_id} marked as approved. Balance updated atomically.")
    except Exception as e:
        logger.error(f"Error in approve deposit execution: {e}")
        await update.message.reply_text("❌ Deposit activation failed safely due to server transaction load.")
...
async def reject_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with db_transaction() as conn:
            ...
            logger.info(f"Deposit #{dep_id} REJECTED for user {user_id}. Reason: {reason}")
        ...
        try:
            ...
            pass
        await update.message.reply_text(f"❌ Deposit ID #{dep_id} flagged as rejected cleanly.")
    except Exception as e:
        logger.error(f"Error in reject deposit execution: {e}")
...
# =====================================================================
# AUDITED CORE DIALOG MANAGEMENT (BUTTON INTERACTION SYSTEM Router)
# =====================================================================
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Phase 1: Deposit Processing Form Flows ---
    ...
    # --- Phase 1: Withdrawal Sequence Steps ---
    ...
    # --- Phase 2: Audited Task Posting State Flow Layout ---
    if task_step == "title":
        if len(text.strip()) < 5:
            await update.message.reply_text("❌ Title too small! Must be at least 5 characters. Retype accurately:")
            return  # Kept in same state explicitly
        ...
        if len(text.strip()) < 10:
            await update.message.reply_text("❌ Instructions require realistic context! Must be at least 10 characters. Retry:")
            return  # Kept in same state explicitly
        ...
        if len(text.strip()) < 5:
            await update.message.reply_text("❌ Validation proof requirements look vague. Please define them clearly:")
            return  # Kept in same state explicitly
        ...
        try:
            ...
            await update.message.reply_text("❌ Reward price must be a valid positive integer. Retry entry:")
            return  # Kept in same state explicitly
        ...
        try:
            ...
            await update.message.reply_text("❌ Slots count must be a valid positive integer. Retry entry:")
            return  # Kept in same state explicitly
        ...
        if text == "✅ Confirm & Post":
            ...
            try:
                ...
                logger.info(f"Task #{task_id} successfully deployed by owner profile {user_id}. Budget deducted: {budget} Tk")
                context.user_data.clear()
    ...
    # --- Phase 2: Text Proof Submission Transition ---
    ...
    # --- Phase 2: Employer Review Note Capture ---
    ...
    # --- Phase 2: Secure Daily Bonus System Claim Logic Block ---
    ...
    # --- Structural Key Buttons Routing Framework Layout ---
    if text == "📊 Dashboard":
        with sqlite3.connect(DATABASE) as conn:
            cursor = conn.cursor()
...
# =====================================================================
# AUDITED ASYNC PHOTO CAPTURE PIPELINE (DEPOSIT SCREENSHOT ROUTER FIX)
# =====================================================================
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dep_step == "dep_step_4":
        ...
        try:
            ...
            logger.info(f"Deposit #{dep_id} submitted atomically by user {user_id}. Amount: {amount} Tk. TxID: {txid}")
            context.user_data.clear()
            ...
            # CRITICAL FIX 1: Send deposit screenshot directly to admin instantly upon submission
            admin_caption = (
            ...
            try:
                ...
                logger.error(f"Critical: Failed to deliver deposit image attachment to admin panel: {e}")
                
        except sqlite3.IntegrityError:
            await update.message.reply_text("❌ Security Block: Duplicate tracking code entry! This transaction/TxID has already been submitted.")
            context.user_data.clear()
        return
