...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_state:
        ...
        if action == "SUPPORT_TICKET_ADMIN_REPLY_INPUT":
            ...
            try:
                ...
                try:
                    await context.bot.send_message(chat_id=row[1], text=f"❌ Your proof for '{row[4]}' was rejected.\n\nReason:\n{text}")
                except: pass
                safe_clear_state(context)
            except Exception as e:
    ...
    if submission_state == "entering_proof_text_fields":
        ...
        # REQUIREMENT 1: Immediate Database Entry & Submission State Synchronization Flow
        try:
            with db_transaction() as conn:
                conn.execute(
    ...
    if text == "📋 Job":
        ...
        if not rows: 
            # --- DEPOSIT GATEWAY ALERT SYSTEM ADDITION ---
            user_msg = (
                "⚠️ বর্তমানে কোনো Payment Method উপলব্ধ নেই।\n\n"
                "⏳ কিছুক্ষণের মধ্যে Payment Method যোগ করা হবে।\n\n"
            ...
            admin_alert = (
                ...
                "একজন User Deposit করতে চেয়েছে কিন্তু কোনো Payment Gateway পাওয়া যায়নি।\n\n"
                f"👤 **User Name:** {u_name}\n"
                f"🆔 **User ID:** `{user_id}`\n"
                f"🕒 **Time:** {now_str}\n\n"
                "💡 Admin Panel (`/admin` -> Finances) থেকে দ্রুত মেথড অ্যাড করুন।"
            )
            ...
            try:
                ...
                for target_admin in admin_ids_set:
                    try:
                        await context.bot.send_message(chat_id=target_admin, text=admin_alert, parse_mode="Markdown")
                    except Exception:
                        pass
            except Exception as outer_err:
                logger.error(f"Failed sending gateway missing alert to admins: {outer_err}")
            return
...
async def handle_admin_sub_nav(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ad_fn_p_w":
        ...
        broadcast_msg = (
            ...
            f"🔍 এখনই Job এ গিয়ে কাজটি সম্পন্ন করুন।"
        )
    ...
    # REQUIREMENT 2 & 6: Task Owner Verification Action Switches (Approve / Reject Proof)
        ...
        try:
            ...
            try: await context.bot.send_message(chat_id=w_id, text="✅ Your proof has been approved.")
            except: pass
...
async def approve_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with db_transaction() as conn:
            row = conn.execute("SELECT status, creator_id, category, reward_amount, total_slots FROM tasks WHERE id = ?", (t_id,)).fetchone()
            if not row or row[0] != 'pending_approval': return await update.message.reply_text("❌ Not found/Already active.")
            conn.execute("UPDATE tasks SET status = 'active' WHERE id = ?", (t_id,))
            
        status, creator_id, category, reward, slots = row
        ...
        # REQUIREMENT 8: Global Broadcast upon successful confirmation activation by system
        public_alert = (
            f"📢 **New Task Added**\n\n"
            f"📂 Category: {category}\n"
            f"🆔 Task ID: {t_id}\n"
...
async def dispute_approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # REQUIREMENT 6: Double Approval/Finalization Protection Guard
    try:
        with db_transaction() as conn:
            ...
            # Warn History Level Calculation System Strategy Integration
        ...
        await update.message.reply_text(f"✅ Dispute #{s_id} Approved. Worker rewarded. Task creator warning level is now: {warn_count}")
        
        # Suggested Warning Levels Execution
        warn_text = "⚠ Warning!\n\nYou rejected a valid proof.\nIf this happens again your account may be suspended or banned."
        if warn_count == 1: warn_text += "\nLevel: Notice"
        elif warn_count == 2: warn_text += "\nLevel: Strong Notice"
        elif warn_count == 3: warn_text += "\nLevel: Temporary Suspension"
        elif warn_count >= 5: warn_text += "\nLevel: Permanent Ban"
        
        try: await context.bot.send_message(chat_id=w_id, text="✅ Admin approved your proof.")
        except: pass
        try: await context.bot.send_message(chat_id=c_id, text=warn_text)
        except: pass
...
async def dispute_reject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Dispute #{s_id} Rejected. Original task owner decision stands.")
